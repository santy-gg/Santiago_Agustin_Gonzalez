<link rel="stylesheet" type='text/css' href="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/devicon.min.css" />
# Santiago_Agustin_Gonzalez
"¡Hola! Soy Santiago Agustín Gonzalez | Backend Developer & Analista de Datos"

### Sobre Mí:
Estudiante avanzado de la Licenciatura en Sistemas.

Apasionado por la arquitectura de software, el diseño de APIs REST y la optimización de bases de datos relacionales.

Enfocado en resolver problemas de negocio mediante código limpio e inteligencia artificial.

### Stack Tecnológico:

### Backend: 
<img height=35px width=35px src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/python/python-original.svg" />
, <img height=35px width=35px src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/java/java-original.svg" />
, <img height=35px width=35px src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/flask/flask-original-wordmark.svg" />
, <img height=35px width=35px src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/javascript/javascript-original.svg" />
, <img height=35px width=35px src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/html5/html5-original.svg" />
, <img height=35px width=35px src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/react/react-original.svg" />.

## Bases de Datos: 
<img height=35px width=35px src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/postgresql/postgresql-original-wordmark.svg" />
, <img height=35px width=35px src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/mongodb/mongodb-original-wordmark.svg" />
, <img height=35px width=35px src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/azuresqldatabase/azuresqldatabase-original.svg" />.

## Datos & BI: 
<img width=35px height=35px src="https://img.icons8.com/color/48/power-bi-2021.png" alt="power-bi-2021"/>
, <img width=35px height=35px src="https://img.icons8.com/ios/50/machine-learning.png" alt="machine-learning"/> Machine Learning
, <img width=35px height=35px src="https://img.icons8.com/color/48/microsoft-excel-2019--v1.png" alt="microsoft-excel-2019--v1"/>.

## Herramientas: 
<img height=35px width=35px src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/github/github-original-wordmark.svg" />
, <img height=35px width=35px src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/docker/docker-original-wordmark.svg" />
, <img height=35px width=35px src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/linux/linux-original.svg" />.

### Proyectos Destacados

#### AINABI - Plataforma Integral ERP/CRM orientada a RRHH
Sistema de gestión empresarial diseñado para optimizar procesos de recursos humanos combinando una arquitectura backend robusta con inteligencia artificial.

**Arquitectura y Tecnologías:**
* **API REST:** Desarrollada íntegramente en Python utilizando el framework Flask para gestionar la capa de servicios.
* **Base de Datos:** Modelado relacional y gestión de transacciones utilizando PostgreSQL.
* **Inteligencia Artificial:** Implementación de modelos predictivos (Machine Learning) integrados directamente en el flujo de trabajo para analizar y predecir el rendimiento de los empleados.
* **Aseguramiento de Calidad (QA):** Definición, ejecución y gestión de casos de prueba manuales en un entorno ágil (Scrum) para garantizar la estabilidad de las entregas.

**Enlaces del Proyecto:**
* 💻 [Repositorio del Backend en GitHub](https://github.com/santy-gg/AINABI_backend.git)
* ⚙️ [Demo Visual / Landing Page](https://ainabiok.netlify.app/)

---

### Contacto
* 💼 [Mi perfil de LinkedIn](www.linkedin.com/in/santiago-agustín-gonzalez-541716263)
